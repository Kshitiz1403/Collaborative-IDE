...
const Monaco = ({ roomId, height, loadingComponent = <CircularProgress /> }) => {
   const { language, updateValue, value, filePath } = useEditor();
   ...
   const handleEditorMount = editor => setEditorRef(editor);

   const handleUpdateValue = value => updateValue(value);

   const getMonacoSubscriberId = () => `${roomId}-${filePath}`;
   ...
   return (
      <Editor
         ...
         defaultLanguage={language == 'nodejs' ? 'javascript' : language == 'python' ? 'python' : languageMap[language]}
         language={language == 'nodejs' ? 'javascript' : language == 'python' ? 'python' : languageMap[language]}
         theme="customTheme"
